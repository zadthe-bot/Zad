...
export const AddressModal: React.FC<AddressModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="white-card p-5 sm:p-6 max-w-lg w-full shadow-2xl relative text-slate-800 border border-slate-300 bg-white rounded-3xl overflow-y-auto max-h-[90vh]">
        <button
          onClick={onClose}
        ...
        <p className="text-xs text-slate-600 font-semibold mb-4">
          Pin your exact delivery location on the vector map or choose a city
        </p>

        {/* Tab Selection */}
        <div className="flex rounded-2xl bg-slate-100 p-1 mb-5 border border-slate-300">
          <button
        ...
        {activeTab === 'map' ? (
          ...
          <div>
            ...
            <div className="mb-5">
              ...
              {errorMsg && (
                <p className="mt-2 text-[11px] text-red-600 font-bold bg-red-50 p-2 rounded-xl border border-red-200">
                  {errorMsg}
                </p>
              )}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-2xl border text-left transition-all cursor-pointer ${
                      isSelected
                        ? 'bg-slate-100 border-emerald-950 text-slate-900 font-black shadow-xs'
                        : 'bg-slate-50 border-slate-300 hover:border-emerald-900 text-slate-800'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2 rounded-xl ${
                          isSelected ? 'bg-emerald-950 text-white' : 'bg-slate-200 text-slate-700'
                        }`}
                      >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-extrabold text-slate-900">{preset.label}</span>
                          {preset.badge && (
                            <span className="px-1.5 py-0.5 rounded-md bg-slate-900 text-white text-[9px] font-black uppercase">
                              {preset.badge}
                            </span>
                          )}
                        </div>
                        <span className="text-[11px] text-slate-600 line-clamp-1">{preset.address}</span>
                      </div>
                    </div>

                    {isSelected && <Check className="w-4 h-4 text-emerald-950 stroke-[3]" />}
                  </button>
                );
              })}
            </div>
            ...
            <form onSubmit={handleCustomSubmit} className="pt-4 border-t border-slate-200">
